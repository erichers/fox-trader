...
const HORIZON_SPEC: Record<Horizon, { dtes: number[]; maxHoldBars: number }> = {
  ...
  swing_daily: { dtes: [3, 4, 5], maxHoldBars: 5 },
  swing_weekly: { dtes: [7, 10], maxHoldBars: 10 },
};

// Exit bounds. tp 0 = no cap (the house profile: −10% cut, lock after +20%, trail the rest).
const EXIT_BOUNDS = { sl: [10, 15], tp: [20, 400], trail: [8, 15] };
...
async function createIdeaBot(env: TradingEnv, idea: ValidIdea, ideaId: number, generation: number, parentId: number | null): Promise<number> {
  ...
  const risk: any = {
    ...
    hold_overnight: true,
    hold_over_weekend: idea.horizon !== 'daytrade',
  };
