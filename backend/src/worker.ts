...
import { ensureSwingFleet } from './risk/swingprofile.js';
...
export function startWorker() {
  void getTradingEnv().then((env) => ensureSwingFleet(env)).catch((e) => console.error('ensureSwingFleet', e?.message || e));
  const sync = loop('sync', 30_000, async () => {
    ...
    await ensureSwingFleet(env).catch((e) => { console.error('ensureSwingFleet(sync)', e?.message || e); });
  ...
  // Live trailing-stop / TP / SL enforcement on open positions (every 15s, market hours).
  ...
  const monitors = loop('monitor', 15_000, async () => {
