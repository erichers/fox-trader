...
import { weekdayDte, MIN_ENTRY_DTE, MAX_ENTRY_DTE } from '../risk/exitpolicy.js';
...
const _optQuoteCache = new Map<string, { bid: number; ask: number; mid: number | null; ts: number }>();
const OPT_QUOTE_TTL = 1000;

/** Latest quotes (bid/ask) for a batch of OCC symbols. Cached 1s so the desk can tick. */
export async function latestQuotes(symbols: string[]): Promise<Record<string, { bid: number; ask: number; mid: number }>> {
  ...
  for (const raw of symbols) {
    ...
    if (!s) continue;
    const hit = _optQuoteCache.get(s);
    if (hit && now - hit.ts < OPT_QUOTE_TTL) {
      out[s] = { bid: hit.bid, ask: hit.ask, mid: hit.mid as any };
    } else {
      need.push(s);
    }
  }
  for (let i = 0; i < need.length; i += 100) {
    ...
    try {
      ...
      for (const [sym, q] of Object.entries<any>(res.quotes || {})) {
        ...
        const mid = bid > 0 && ask > 0 ? Math.round(((bid + ask) / 2) * 100) / 100 : (null as any);
        const row = { bid, ask, mid };
        out[sym] = row;
        _optQuoteCache.set(sym, { ...row, ts: now });
      }
    } catch { /* skip batch */ }
  }
  if (_optQuoteCache.size > 400) {
    const old = [..._optQuoteCache.entries()].sort((a, b) => a[1].ts - b[1].ts).slice(0, 200);
    for (const [k] of old) _optQuoteCache.delete(k);
  }
  return out;
}
 ...
 *  Swing desk: weekly/daily/default land in 2–10 trading days (aim ~7). Monthly is the
 *  far edge of that window, not 30-day paper. LEAPS still pick the longest listed date. */
function pickExpiration2(exps: string[], pref: string): string {
  const today = new Date().toLocaleDateString('en-CA', { timeZone: 'America/New_York' });
  const dteCal = (e: string) => (Date.parse(e + 'T00:00:00Z') - Date.now()) / 864e5;
  const notZero = exps.filter((e) => e > today);
  const pool = notZero.length ? notZero : exps;
  if (/^\d{4}-\d{2}-\d{2}$/.test(pref)) return pool.find((e) => e >= pref) || pool[pool.length - 1];
  const swing = pool.filter((e) => {
    const w = weekdayDte(e, today);
    return w >= MIN_ENTRY_DTE && w <= MAX_ENTRY_DTE;
  });
  if (pref === 'leaps') return pool.reduce((b, e) => (dteCal(e) > dteCal(b) ? e : b));
  const inWindow = swing.length ? swing : pool.filter((e) => dteCal(e) >= 2);
  if (!inWindow.length) return pool[pool.length - 1];
  const target = pref === 'monthly' ? MAX_ENTRY_DTE : 7;
  return inWindow.reduce((b, e) => (Math.abs(weekdayDte(e, today) - target) < Math.abs(weekdayDte(b, today) - target) ? e : b));
}
...
export async function resolveContract(
  ...
  const exp = pickExpiration2(exps, expirationPref || 'weekly');
