...
import { GAIN_LOCK_ARM_PCT, GAIN_LOCK_FLOOR_PCT, RATCHET_0, RATCHET_1, RATCHET_2, HARD_STOP_PCT, FLEX_STOP_PCT } from './risk/exitpolicy.js';
...
export type TradeRow = {
  ...
  qty: number; cost_usd: number; value_usd: number | null; pl_usd: number;
...
function suggestFor(t: { pl: number; peak: number; reason: string; status: string; sl: number; trail: number; tp: number }): string {
  ...
  if (reason === 'gain-lock' || reason === 'breakeven-lock') return `Gain lock: peaked +${r1(peak)}% and the lock closed it at ~+${GAIN_LOCK_FLOOR_PCT}% instead of giving it back.`;
  ...
  if (reason === 'trailing-stop' && giveback > 14) return `Rode to +${r1(peak)}% but gave back ${r1(giveback)}pts to the trail. The ratchet (live now) tightens the trail to ${RATCHET_0.trail}pts after +${RATCHET_0.at}%, ${RATCHET_1.trail}pts after +${RATCHET_1.at}%, and ${RATCHET_2.trail}pts after +${RATCHET_2.at}% — this exit improves to ~+${r1(Math.max(pl, peak - (peak >= RATCHET_2.at ? RATCHET_2.trail : peak >= RATCHET_1.at ? RATCHET_1.trail : RATCHET_0.trail)))}%.`;
...
export async function tradeAnalysis(opts: { days?: number; envArg?: TradingEnv } = {}): Promise<any> {
  ...
  const trades: TradeRow[] = mons.map((m: any) => {
    ...
    const sl = Number(m.sl_pct) || 0, trail = Number(m.trail_pct) || 0, tp = Number(m.tp_pct) || 0;
      ...
      const opt = String(m.asset_class || '').toLowerCase() === 'option';
      const mult = opt ? 100 : 1;
      const mark = m.status === 'closed' ? (Number(m.exit_price) > 0 ? Number(m.exit_price) : Number(m.last_price)) : Number(m.last_price);
      const cost_usd = Math.round(entry * qty * mult * 100) / 100;
      const value_usd = mark > 0 ? Math.round(mark * qty * mult * 100) / 100 : null;
      const pl_usd = value_usd != null ? Math.round((value_usd - cost_usd) * 100) / 100 : 0;
      const row: TradeRow = {
  ...
  // ── Per-day P/L ────────────────────────────────────────────────────────────
  const byDay: Record<string, { date: string; n: number; wins: number; pl_sum: number; pl_usd: number }> = {};
  for (const t of trades) {
    ...
    (byDay[d] ||= { date: d, n: 0, wins: 0, pl_sum: 0, pl_usd: 0 });
    byDay[d].n++; if (t.verdict === 'win') byDay[d].wins++;
    byDay[d].pl_sum = r1(byDay[d].pl_sum + t.pl_pct);
    byDay[d].pl_usd = Math.round((byDay[d].pl_usd + t.pl_usd) * 100) / 100;
  }
  ...
  for (const bid of botIds) {
    ...
    if (n >= 6) {
      ...
      if (losses.length >= 3 && cleanLosses / losses.length >= 0.7 && curSl > HARD_STOP_PCT) {
        recSl = Math.max(HARD_STOP_PCT, r1(curSl - (curSl - avgLoss * 0.85) / 2));
        ...
        recSl = Math.min(FLEX_STOP_PCT, r1(Math.max(curSl, HARD_STOP_PCT) * 1.1));
        rationale.push(`${whipsaws} losers were up ≥15% before stopping out (whipsaw) — robust setups may use a ${FLEX_STOP_PCT}% flex stop; nothing wider. The +${GAIN_LOCK_ARM_PCT}% gain lock handles the rest.`);
      }
      ...
      if (wins.length >= 3) {
        ...
        if (gbShare > 0.6 && curTrail > 12) {
          recTrail = Math.max(8, r1(curTrail - (curTrail - median(gb)) / 2));
      ...
      recSl = Math.min(FLEX_STOP_PCT, Math.max(HARD_STOP_PCT, recSl || HARD_STOP_PCT));
      recTrail = Math.min(15, Math.max(8, recTrail || 12));
    ...
    bots.push({
      ...
      total_pl_usd: Math.round(rows.reduce((s, t) => s + t.pl_usd, 0) * 100) / 100,
  ...
  return {
    ...
    summary: {
      ...
      total_pl_usd: Math.round(closed.reduce((s, t) => s + t.pl_usd, 0) * 100) / 100,
    ...
    note: 'Every row is a real monitored round-trip: entry fill → actual exit (≈ marks a trigger price where the broker reported no fill). Peak = the best the trade ever looked; giveback = peak − realized. Suggestions reference the +20% gain lock + progressive trail now live in both the monitor and the backtest.',
  };
}
    ...
    if (opts.apply && rec.changed && rec.confidence !== 'low') {
      ...
      if (bot) {
        ...
        if (action._quickbot) {
          ...
          const clampPct = (v: number) => Math.min(FLEX_STOP_PCT, Math.max(HARD_STOP_PCT, Math.round(v * 10) / 10));
