...
export async function tradeDefaultsFactory(): Promise<TradeDefaults> {
  ...
  return {
    ...
    max_usd: max,
    take_profit_pct: 25,  // soft goal — prefer close when hit; trail still rides
    stop_loss_pct: 10,
...
export function clampTradeDefaults(next: Partial<TradeDefaults>, base: TradeDefaults): TradeDefaults {
  ...
  if (!sl && !trail) sl = base.stop_loss_pct || 10;
...
/** Global default position-holding policy. Eric 2026-09-12: NO overnight / NO weekend on
 *  the swing desk. Flatten in the close buffer. LEAPS override via risk.hold_overnight /
 *  risk.hold_over_weekend = true. `closeBufferMin` is minutes before the close. */
...
export async function getExitPolicy(): Promise<ExitPolicy> {
  ...
  return {
    holdOvernight: o?.holdOvernight === true,           // default false — flatten same day
    holdOverWeekend: o?.holdOverWeekend === true,       // default false
