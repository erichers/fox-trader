...
import { bandForDte, clampEntryDte, isLeapsBot } from '../risk/swingprofile.js';
...
export async function evaluateBot(botRow: any): Promise<any> {
  const botClass = String(botRow.asset_class || 'equity').toLowerCase();
  if (botClass !== 'option') {
    return [{ skipped: 'equity bot — desk is options-only', asset_class: botClass }];
  }
    ...
    if (ev.fired && aiOk) {
      ...
      try {
        ...
        if (isOption) {
          ...
          if (isLeapsBot(bot.action, bot.name)) {
            draft.expiration = bot.action?.expiration || 'leaps';
          } else {
            const dte = Number.isFinite(Number(bot.action?._dte)) ? clampEntryDte(Number(bot.action._dte)) : 7;
            draft.expiration = dteToExpiration(dte);
            const band = bandForDte(dte);
            draft._play = { dte, tp: band.tp, sl: band.sl, trail: band.trail, maxPositionUsd: band.max_position_usd };
          }
