...
import { applySwingFleet, isLeapsBot } from '../risk/swingprofile.js';
import { refreshNews } from '../market/news.js';
import { getBars } from '../brokers/index.js';
import { expirations as optExpirations, getChain, contractPrice, latestQuotes } from '../brokers/options.js';
...
function validateDraft(b: any): string | null {
  ...
  const ac = String(b.asset_class || 'option').toLowerCase();
  if (b.asset_class && !['equity', 'etf', 'option'].includes(ac)) return 'invalid asset_class';
  if (ac !== 'option' && b.side === 'buy') return 'equity/etf buys are blocked — options only';
  if (ac === 'option' && b.option_type && !['call', 'put'].includes(b.option_type)) return 'invalid option_type';
  return null;
}

function J(v: any, d: any) {
  if (v == null) return d;
  if (typeof v === 'object') return v;
  try { return JSON.parse(v); } catch { return d; }
}

function optLast(q: { bid: number; ask: number; mid: number } | undefined): number | null {
  if (!q) return null;
  if (Number(q.mid) > 0) return Number(q.mid);
  if (Number(q.ask) > 0) return Number(q.ask);
  if (Number(q.bid) > 0) return Number(q.bid);
  return null;
}

/** Overlay live last / value / P/L on the current env's positions. Options use OCC quotes. */
async function livePositionRows(): Promise<any[]> {
  ...
  const occs = rows.filter((r) => (r.asset_class || '') === 'option' && r.occ_symbol && Number(r.qty)).map((r) => String(r.occ_symbol).toUpperCase());
  const optQ = occs.length ? await latestQuotes(occs).catch(() => ({} as Record<string, any>)) : {};
  for (const r of rows) {
    const ac = (r.asset_class || 'equity').toLowerCase();
    if (ac === 'option') {
      const last = optLast(optQ[String(r.occ_symbol || '').toUpperCase()]);
      const qty = Number(r.qty) || 0;
      if (last != null && qty) {
        r.last_price = last;
        r.market_value = Math.round(last * qty * 100 * 100) / 100;
        if (r.avg_cost != null) r.unrealized_pl = Math.round((last - Number(r.avg_cost)) * qty * 100 * 100) / 100;
        r.cost_usd = Math.round(Number(r.avg_cost || 0) * qty * 100 * 100) / 100;
        r.live = true;
      } else if (qty && r.avg_cost != null) {
        r.cost_usd = Math.round(Number(r.avg_cost) * qty * 100 * 100) / 100;
      }
    } else {
      const last = prices[String(r.symbol).toUpperCase()];
      const qty = Number(r.qty) || 0;
      if (qty && r.avg_cost != null) r.cost_usd = Math.round(Number(r.avg_cost) * qty * 100) / 100;
      if (last != null && qty) {
        r.last_price = last;
        r.market_value = Math.round(last * qty * 100) / 100;
        if (r.avg_cost != null) r.unrealized_pl = Math.round((last - Number(r.avg_cost)) * qty * 100) / 100;
        r.live = true;
      }
    }
  }
...
function occFromOrderRaw(raw: any): string {
  const draft = J(raw, {}).draft || {};
  return String(draft?._contract?.occSymbol || draft?._contract?.occ_symbol || '').toUpperCase();
}

function orderCostUsd(o: any): number | null {
  const qty = Number(o.qty) || 0;
  const draft = J(o.raw, {}).draft || {};
  const px = Number(o.filled_price) || Number(o.limit_price) || Number(draft.est_price) || 0;
  if (!(qty > 0) || !(px > 0)) return null;
  const opt = String(o.asset_class || '').toLowerCase() === 'option' || !!occFromOrderRaw(o.raw);
  return Math.round(qty * px * (opt ? 100 : 1) * 100) / 100;
}

function waitingFromBot(b: any): { id: number; name: string; mode: string; symbols: string[]; last_evaluated_at: any; state: string; why: string } {
  const symbols: string[] = J(b.symbols, []);
  const rows: any[] = Array.isArray(J(b.last_result, [])) ? J(b.last_result, []) : [];
  const acted = rows.find((r) => r?.fired && (r.status === 'placed' || r.status === 'filled' || r.action === 'execute'));
  const first = rows.find((r) => r && !r.error) || rows[0];
  return {
    id: Number(b.id),
    name: String(b.name || `bot #${b.id}`),
    mode: String(b.mode || 'observe'),
    symbols,
    last_evaluated_at: b.last_evaluated_at || null,
    state: acted ? 'fired' : 'waiting',
    why: String(acted?.why || first?.why || first?.skipped || 'waiting for next signal').slice(0, 180),
  };
}
...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.get('/api/positions', async () => livePositionRows());
  // One round-trip for the always-on desk strip: live positions, open monitors,
  // bots waiting on a signal, recent fills. Polled every 1–3s while the tab is visible.
  app.get('/api/desk', async () => {
    const env = await getTradingEnv();
    const [positions, monitors, bots, orders, alerts, clock, defaults, limits] = await Promise.all([
      livePositionRows(),
      q<any>("SELECT * FROM position_monitors WHERE status='open' AND (env=:env OR (env IS NULL AND :env='alpaca_paper')) ORDER BY opened_at DESC LIMIT 50", { env }),
      q<any>("SELECT id, name, enabled, mode, symbols, last_evaluated_at, last_result, asset_class, action, rules FROM bots WHERE env=:env AND enabled=1 AND mode IN ('cautious','auto','full_auto') ORDER BY id ASC", { env }),
      q<any>("SELECT id, symbol, side, qty, status, source, bot_id, created_at, filled_price, limit_price, asset_class, raw FROM orders WHERE env=:env AND asset_class='option' AND status IN ('placed','filled','staged') ORDER BY id DESC LIMIT 20", { env }),
      q<any>("SELECT id, level, source, title, body, symbol, created_at, status FROM alerts WHERE status='new' ORDER BY id DESC LIMIT 12"),
      getClock().catch(() => ({ is_open: false })),
      getTradeDefaults(),
      getRiskLimits(),
    ]);
    const occQuotes: Record<string, number> = {};
    for (const p of positions) {
      if (p.occ_symbol && p.last_price != null) occQuotes[String(p.occ_symbol).toUpperCase()] = Number(p.last_price);
    }
    const botName = new Map(bots.map((b: any) => [Number(b.id), String(b.name)]));
    for (const m of monitors) {
      const occ = String(m.occ_symbol || '').toUpperCase();
      if (occ && occQuotes[occ] != null) { m.last_price = occQuotes[occ]; m.live = true; }
      m.bot_name = botName.get(Number(m.bot_id)) || null;
    }
    for (const p of positions) {
      const m = monitors.find((x: any) => String(x.occ_symbol || '') && x.occ_symbol === p.occ_symbol);
      p.bot_id = p.bot_id ?? m?.bot_id ?? null;
      p.bot_name = botName.get(Number(p.bot_id || m?.bot_id)) || null;
      p.monitor_id = m?.id ?? null;
    }
    const ordersOut = orders.map((o: any) => {
      const occ = occFromOrderRaw(o.raw);
      return {
        id: o.id, symbol: o.symbol, side: o.side, qty: o.qty, status: o.status, source: o.source,
        bot_id: o.bot_id, created_at: o.created_at, filled_price: o.filled_price, limit_price: o.limit_price,
        asset_class: o.asset_class, occ_symbol: occ || null, cost_usd: orderCostUsd(o),
        bot_name: botName.get(Number(o.bot_id)) || null,
      };
    });
    return {
      ts: Date.now(), env,
      market_open: !!(clock as any)?.is_open,
      positions, monitors,
      waiting: bots.map(waitingFromBot),
      orders: ordersOut, alerts,
      trade_defaults: defaults,
      limits,
      mode: await getGlobalMode(),
    };
  });
  // Live save: global mode, $ per trade, and optionally arm every option bot on full-auto.
  app.post('/api/desk/policy', async (req) => {
    const b = (req.body as any) || {};
    const env = await getTradingEnv();
    const out: any = { env };
    if (b.mode && MODES.includes(b.mode)) {
      await setSetting('global_mode', b.mode);
      out.mode = b.mode;
      await audit('mode.change', `global mode → ${b.mode}`);
    }
    if (b.amount_usd != null || b.max_usd != null) {
      out.trade_defaults = await setTradeDefaults({
        amount_usd: b.amount_usd ?? undefined,
        max_usd: b.max_usd ?? b.amount_usd ?? undefined,
      });
    }
    const cap = Number(b.maxPositionUsd ?? b.amount_usd);
    if (Number.isFinite(cap) && cap > 0) {
      out.limits = await setRiskLimits({ maxPositionUsd: cap });
    }
    if (b.all_bots_full_auto) {
      const bots = await q<any>('SELECT id, name, action, rules, risk, asset_class FROM bots WHERE env=:env', { env });
      const mode = (b.mode && MODES.includes(b.mode) ? b.mode : 'full_auto') as Mode;
      let n = 0;
      for (const bot of bots) {
        const action = J(bot.action, {});
        const rules = J(bot.rules, {});
        if (action._observe_only || rules._observe_only) continue;
        if (String(bot.asset_class || '').toLowerCase() !== 'option') continue;
        if (isLeapsBot(action, bot.name)) continue;
        let risk = J(bot.risk, {});
        if (Number.isFinite(cap) && cap > 0) {
          // Desk $ amount is a CEILING. Never copy it onto every DTE band (that is how
          // $10k landed on 1-DTE tickets on 2026-09-11).
          risk = { ...risk, max_position_usd: cap };
        }
        await exec('UPDATE bots SET enabled=1, mode=:m, risk=CAST(:r AS JSON) WHERE id=:id AND env=:env', {
          m: mode, r: JSON.stringify(risk), id: bot.id, env,
        });
        n++;
      }
      out.bots_armed = n;
      await audit('desk.policy.bots', `${n} option bots → ${mode} cap=${cap || 'unchanged'}`);
    }
    out.swing = await applySwingFleet(env);
    return out;
  });
  ...
  app.get('/api/orders', async (req) => {
    ...
    return q("SELECT * FROM orders WHERE env=:env AND asset_class='option' ORDER BY created_at DESC LIMIT :lim", { env, lim });
  });
