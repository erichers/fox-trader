...
import { expirationIsZeroDte, occExpiration, weekdayDte, buyDteAllowed } from './exitpolicy.js';
...
export type OrderDraft = {
  ...
  _play?: { name?: string; tag?: string; dte?: number; tp?: number; sl?: number; trail?: number; maxPositionUsd?: number; allow_0dte?: boolean };
  ...
  // Same-day expiry buys are refused unless this is explicitly true (irresistible setup).
  _allow_0dte?: boolean;
};
...
export async function riskCheck(draft: OrderDraft, env?: TradingEnv, mode?: Mode): Promise<RiskResult> {
  ...
  // 3. Options-only desk. Equity/ETF buys are permanently blocked (ignore allowlist).
  //    Close-only sells of leftover stock still pass so the book can flatten.
  const optionsOnlyBuy = ac === 'option' || draft.side === 'sell';
  checks.asset_class = {
    pass: optionsOnlyBuy,
    detail: ac === 'option' ? ac
      : draft.side === 'sell' ? `${ac} sell — flatten leftover stock (desk is options-only)`
      : 'equity/etf buys are blocked — this desk trades options only',
  };
  ...
  //    NEVER blocks a close-only SELL. 2026-09-11: 0DTE stops fired at -35% then this
  //    check vetoed the exits (18% down vs 3% cap), so losers sat at -40%.
  let dailyOk = true; let dailyDetail = 'no equity snapshot — skipped';
  if (draft.side === 'sell') {
    dailyOk = true;
    dailyDetail = 'sell/exit — daily loss does not block closes';
  } else if (isLiveEnv(env) && draft.side === 'buy' && !(equity > 0)) {
    ...
    if (!startEq) { startEq = equity; await setSetting(dayKey, startEq); }
  ...
  // 7b. Swing DTE window. Buys must be 2–14 trading days to expiry. NEVER 0DTE / 1DTE
  //     (allow_0dte is ignored — Eric 2026-09-12). Sells always pass so leftovers can flatten.
  const exp = draft._contract?.expiration
    || occExpiration(draft._contract?.occSymbol)
    || (/^\d{4}-\d{2}-\d{2}$/.test(String(draft.expiration || '')) ? String(draft.expiration) : null);
  const zeroDte = ac === 'option' && expirationIsZeroDte(exp);
  const dte = ac === 'option' && exp ? weekdayDte(exp) : 0;
  const dteGate = ac === 'option' && draft.side === 'buy' && exp
    ? buyDteAllowed(dte, false)
    : { ok: true, detail: draft.side === 'sell' ? 'sell/exit — DTE window does not block closes' : (!exp && ac === 'option' && draft.side === 'buy' ? 'expiration unresolved — deferred' : 'not option') };
  const zeroDteBuy = draft.side === 'buy' && zeroDte;
  checks.zero_dte = {
    pass: !zeroDteBuy,
    detail: draft.side === 'sell' ? 'sell/exit — 0DTE close allowed'
      : zeroDteBuy ? `0DTE buy blocked (${exp} expires today). Never 0DTE.`
      : 'not 0DTE',
  };
  checks.swing_dte = {
    pass: dteGate.ok,
    detail: dteGate.detail,
  };
