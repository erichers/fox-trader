...
import { exitReason, occIsZeroDte, HARD_STOP_PCT, FLEX_STOP_PCT, MAX_HOLD_SESSIONS, weekdayDte } from './exitpolicy.js';
...
export async function createMonitor(draft: OrderDraft, orderId: number, entryPrice: number, env?: TradingEnv): Promise<void> {
  ...
  if (ac === 'option') sl = sl > 0 ? Math.min(sl, occIsZeroDte(occ) ? HARD_STOP_PCT : FLEX_STOP_PCT) : HARD_STOP_PCT;
  // AI-opened positions have no per-bot risk config — give them the swing defaults so they
  // are never left unmanaged. Cut losers at −10%, no take-profit cap, trail after the +20% lock.
  if (draft.source === 'ai' && !tp && !sl && !trail) {
    if (ac === 'option') { tp = 0; sl = HARD_STOP_PCT; trail = 12; } else { tp = 0; sl = 8; trail = 12; }
  }
...
export async function checkMonitors(opts?: { reconcileOnly?: boolean }): Promise<{ checked: number; closed: number }> {
  ...
  for (const m of open) {
    ...
    let reason = exitReason(fav, peakFav, { tp: Number(m.tp_pct) || 0, sl: Number(m.sl_pct) || 0, trail: Number(m.trail_pct) || 0 }, {
      zeroDte: isOption && occIsZeroDte(m.occ_symbol),
      option: isOption,
      flex: isOption && Number(m.sl_pct) > HARD_STOP_PCT,
    }) || '';
    // Expiry-day flatten fires the whole session (not only the close buffer). A 0DTE leftover
    // must not sit until 3:45 hoping the −10% stop is enough against gamma.
    if (!reason && isOption && occIsZeroDte(m.occ_symbol)) reason = 'flatten: contract expires today';
    ...
    if (!reason && nearClose) {
      ...
      // Hard limits that OVERRIDE any hold policy: a long option is always sold on its
      // expiration day, and an idea bot's max hold is enforced live. Swing options without
      // an explicit max default to 10 sessions (~2 weeks). LEAPS (remaining DTE > 10+2)
      // keep whatever the bot set (0 = no time stop).
      const etToday = new Date().toLocaleDateString('en-CA', { timeZone: 'America/New_York' });
      const c = isOption && m.occ_symbol ? occToContract(m.occ_symbol) : null;
      const remaining = c?.expiration ? weekdayDte(c.expiration, etToday) : 0;
      if (isOption && maxHoldBars <= 0 && remaining > 0 && remaining <= MAX_HOLD_SESSIONS + 2) {
        maxHoldBars = MAX_HOLD_SESSIONS;
      }
...
/** Any 0DTE long without a monitor still gets the -10% floor (manual fills, missed create). */
async function ensureZeroDteStops(env: string): Promise<void> {
  const held = await q<any>("SELECT symbol, occ_symbol, qty, avg_cost FROM positions WHERE env=:env AND asset_class='option' AND qty>0", { env });
  for (const p of held) {
    const occ = String(p.occ_symbol || '');
    if (!occIsZeroDte(occ)) continue;
    const [mon] = await q<{ id: number }>("SELECT id FROM position_monitors WHERE status='open' AND (env=:env OR env IS NULL) AND occ_symbol=:occ LIMIT 1", { env, occ });
    if (mon) continue;
    const entry = Number(p.avg_cost) || 0;
    if (!(entry > 0)) continue;
    await exec(
      `INSERT INTO position_monitors (symbol, occ_symbol, env, account_number, asset_class, qty, entry_price, peak_price, trough_price, last_price, tp_pct, sl_pct, trail_pct, bot_id, order_id)
       VALUES (:s,:occ,:env,'agentic','option',:qty,:entry,:entry,:entry,:entry,0,:sl,20,NULL,NULL)`,
      { s: p.symbol, occ, env, qty: Number(p.qty) || 1, entry, sl: HARD_STOP_PCT },
    );
    await audit('monitor.open', `0DTE floor on ${occ} from ${entry} (sl ${HARD_STOP_PCT})`);
  }
}
