...
import { DTE_BANDS, QUICK_DTES, bandForDte, clampEntryDte } from './risk/swingprofile.js';
...
export { DTE_BANDS, QUICK_DTES };

/**
 * QUICKBOTS — single-underlying (or tight-basket) swing options bots that trade
 * calls AND puts based on signals, with risk that scales by days-to-expiry (2–10
 * trading days). The default strategy for each is chosen EMPIRICALLY from a 1-year
 * backtest over a grid of signal × direction × DTE combinations (see backtestQuickbot).
 * Everything here is modeled from REAL underlying price action — exact historical
 * short-DTE option fills aren't sourceable, so option P/L is a documented BSM-style
 * approximation (gamma leverage + theta, both scaling by 1/√DTE). Honest by
 * construction; labeled as modeled wherever shown.
 */

// DTE risk bands live in risk/swingprofile.ts (shared with the live fleet rewriter).
// Shorter DTE = smaller size (gamma). Stop and trail are the swing profile: −10% cut,
// trail 12 after the +20% gain lock. tp=0 = no cap.
...
function simulatePlay(closes: number[], times: number[], snaps: any[], start: number, end: number,
  ...
  for (let i = start; i <= end; i++) {
    ...
    if (open) {
      ...
      let reason = exitReason(oret, open.peak, band, { option: true, zeroDte: dte <= 1 || (dte - held) < 1, flex: band.sl > 10 }) || '';
...
export async function backtestIdeaPlay(
  ...
  symbol = symbol.toUpperCase();
  const days = opts.days ?? 365;
  const dte = clampEntryDte(spec.dte);
  ...
  for (const symRaw of symbols) {
    ...
    try {
      ...
      for (const play of plays) {
        if (!QUICK_DTES.includes(Number(play.dte))) continue;
      ...
      for (const play of fired) {
        ...
        try {
          ...
          const band = bandForDte(play.dte, !!play.robust);
          const dte = clampEntryDte(play.dte);
          const draft: OrderDraft = {
            ...
            expiration: dteToExpiration(dte), est_price: undefined, source: 'bot', bot_id: botRow.id,
            _play: { name: play.name, tag: `${play.direction}-${dte}`, dte, tp: band.tp, sl: band.sl, trail: band.trail, maxPositionUsd: band.max_position_usd },
          };
...
export async function runLeaderboardPlay(env: TradingEnv, opts: { symbol: string; key: string; dte: number; mode?: string; robust?: boolean; backtest?: any }): Promise<any> {
  ...
  const dte = clampEntryDte(Number(opts.dte));
  const band = bandForDte(dte, !!opts.robust);
  ...
  const risk = { override: true, max_position_usd: Math.max(...allPlays.map((p) => p.risk.max_position_usd), 2000), max_concentration_pct: 35, max_daily_loss_pct: 25, max_orders_per_day: 20, dte_bands: DTE_BANDS, hold_overnight: true, hold_over_weekend: true };
...
export async function seedQuickbots(env: TradingEnv, opts: { force?: boolean; tuned?: boolean } = {}): Promise<any> {
  ...
  for (const spec of specs) {
    ...
    const risk = {
      ...
      hold_overnight: true, hold_over_weekend: true,
    };
