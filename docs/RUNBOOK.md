...
Checkout lives at **``$HOME/Sites/grokbot/grokbot-rh-trader` (or this fox-trader clone)`**. That is a folder inside the grokbot Sites root, not the grokbot decks tree and not `grokbot-app`.

```bash
# Move `$HOME/Sites/rh.tradingbot` here if needed, wire MAMP, start paper,
...
The desk used to live at ``$HOME/Sites/rh.tradingbot``. `./scripts/install-mac-grokbot.sh` moves that folder to ``$HOME/Sites/grokbot/grokbot-rh-trader` (or this fox-trader clone)` (and will also pick up a misnamed `grokbot/rh-trader` or sibling `Sites/grokbot-rhtrader`) and keeps the old MAMP alias as a bookmark.
