...
import { Card, money, num, signClass, statusBadge, useAsync, Badge, FleetBadge, Progress, Sym, Info, Live, costUsd, notionalUsd, pct } from '../components/ui';
...
function PerformanceBlock() {
  const perf = useAsync<any>(BotPerformance, [], 5000);
  const d = perf.data;
...
export default function Dashboard({ health }: { health: any }) {
  const acct = useAsync(Account, [], 3000);
  const pos = useAsync<any[]>(Positions, [], 3000);
  const orders = useAsync<any[]>(() => Orders(20), [], 5000);
  const appr = useAsync<any[]>(Approvals, [], 5000);
  const botList = useAsync<any[]>(Bots, [], 5000);
  const mons = useAsync<any[]>(Monitors, [], 3000);
  const camp = useAsync<any>(Campaign, [], 15000);
  const clock = useAsync<any>(Clock, [], 30000);
  const pnl = useAsync<any>(Pnl, [], 5000);
    ...
    items.push({ id: 'monitors', title: <>Live risk monitors ({openMons.length}) — enforcing exits<Info topic="monitors" /></>, node: (
      <>
        ...
        <DataTable rows={openMons} storageKey="dash-monitors" filter={openMons.length > 6} cols={[
          ...
          { key: 'pl', label: 'P/L', align: 'right', sortValue: (m) => Number(notionalUsd(m.qty, m.last_price, m) || 0) - Number(notionalUsd(m.qty, m.entry_price, m) || 0), render: (m) => {
            const cost = notionalUsd(m.qty, m.entry_price, m);
            const val = notionalUsd(m.qty, m.last_price, m);
            const usd = cost != null && val != null ? val - cost : null;
            const pl = m.entry_price ? ((Number(m.last_price) - Number(m.entry_price)) / Number(m.entry_price)) * 100 : 0;
            return <span className={signClass(usd ?? pl)}>{usd != null ? money(usd) : '—'} <span className="muted">{pct(pl)}</span></span>;
          } },
  ...
  items.push({ id: 'positions', title: <>Positions ({positions.length}){positions.some((p) => p.live) ? <> <Live /></> : null}</>, node: (
    positions.length === 0 ? <div className="muted">No positions yet. Connect Robinhood and Sync.</div> : (
      <DataTable rows={positions} storageKey="dash-positions" filter={positions.length > 6} filterPlaceholder="filter positions…" cols={[
        ...
        { key: 'cost', label: 'Cost', align: 'right', sortValue: (p) => Number(costUsd(p) || 0), render: (p) => money(costUsd(p)) },
