...
function BotRow({ bot, health, reload, defaultOpen = false, focus = false, autoEdit = false }: { bot: any; health: any; reload: () => void; defaultOpen?: boolean; focus?: boolean; autoEdit?: boolean }) {
  ...
  const startEdit = () => setEdit({
    ...
    stop_loss_pct: J(bot.risk, {}).stop_loss_pct ?? bot.effective_risk?.stop_loss_pct ?? (bot.asset_class === 'option' ? 10 : 8),
    trailing_stop_pct: J(bot.risk, {}).trailing_stop_pct ?? bot.effective_risk?.trailing_stop_pct ?? (bot.asset_class === 'option' ? 12 : 12),
