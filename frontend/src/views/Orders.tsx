...
import { Orders, Approvals, ApproveOrder, RejectOrder, PlaceOrder, RiskCheck, TradeAnalysis, TuneBots, Desk } from '../api/client';
import { Card, money, num, pct, signClass, statusBadge, useAsync, Sym, Info, Badge, fmtDateTime, costUsd } from '../components/ui';
...
function AnalysisSection() {
  ...
  const { data, loading, reload } = useAsync<any>(() => TradeAnalysis(days), [days], 5000);
  ...
  const tradeCols: Column<any>[] = [
    ...
    { key: 'cost_usd', label: 'Cost', align: 'right', sortValue: (t) => Number(t.cost_usd || 0), render: (t) => money(t.cost_usd) },
    { key: 'pl_usd', label: 'P/L $', align: 'right', sortValue: (t) => Number(t.pl_usd || 0), render: (t) => <b className={signClass(t.pl_usd)}>{money(t.pl_usd)}</b> },
    { key: 'pl_pct', label: 'P/L %', align: 'right', sortValue: (t) => t.pl_pct, render: (t) => <span className={signClass(t.pl_pct)}>{pct(t.pl_pct)}</span> },
        ...
        {loading && !data ? <div className="muted">Analyzing trades…</div> : (
          <div className="stack" style={{ gap: 12 }}>
            <div className="row" style={{ gap: 10, flexWrap: 'wrap' }}>
              ...
              <Chip label="Total P/L" value={money(s.total_pl_usd)} kind={signClass(s.total_pl_usd)} />
              {(d.daily || []).map((day: any) => <Chip key={day.date} label={day.date.slice(5)} value={money(day.pl_usd)} kind={signClass(day.pl_usd)} sub={`${day.wins}/${day.n} wins`} />)}
            </div>
            ...
            <DataTable rows={trades.filter((t) => t.status !== 'open')} cols={tradeCols} storageKey="trade-analysis" initialSort={{ key: 'closed_at', dir: -1 }} pageSize={20} filterPlaceholder="filter symbol/bot/exit…" empty="No closed round-trips in this window." rowKey={(t) => t.id} />
            {sel && (
...
export default function OrdersView() {
  ...
  const desk = useAsync<any>(Desk, [], 3000);
  ...
  const doPlace = async () => {
    ...
    try {
      ...
      orders.reload(); appr.reload(); desk.reload();
    } catch (e: any) { setMsg('Order rejected: ' + String(e)); }
  };

  return (
    <div className="grid" style={{ gap: 14 }}>
      <Card title="Open book">
        <OpenBook
          positions={(desk.data?.positions || []).filter((p: any) => Number(p.qty) > 0)}
          waiting={desk.data?.waiting || []}
          live={desk.data?.env && desk.data.env !== 'alpaca_paper'}
          onSold={() => { desk.reload(); orders.reload(); }}
          show="both"
        />
      </Card>
      <AnalysisSection />
      ...
      <Card title={`Order History (${(orders.data || []).length})`}>
        <DataTable
          ...
          cols={[
            ...
            { key: 'cost', label: 'Cost', align: 'right', sortValue: (o) => Number(costUsd(o) || 0), render: (o) => { const c = costUsd(o); return c != null ? money(c) : <span className="muted">mkt</span>; } },
