...
const bandFor = (bands: any, dte: number) => bands?.[dte] || bands?.[String(dte)] || { tp: 0, sl: 10, trail: 12, max_position_usd: 2500, qty: 1 };
...
function PlaysManager({ bot, signals, reload }: { bot: any; signals: any; reload: () => void }) {
  ...
  const dtes: number[] = signals?.dtes || [2, 3, 4, 5, 7, 10];
      ...
      <details>
        ...
        <div style={{ marginTop: 8 }}>
          <div className="muted" style={{ fontSize: 12, lineHeight: 1.5 }}>
            ...
            market hours; when a play's signal fires it buys that option (deduped to once/day per contract). <b>Swing profile:</b>{' '}
            2–10 trading days, cut losers at −10%, no trail until the trade is up 20% (room for a good setup), then lock at least +5% and
            trail tighter as the peak grows. Shorter DTE still gets smaller size so a $10k desk cap cannot reload 2-day gamma.
          </div>
