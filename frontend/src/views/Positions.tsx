...
import { Card, money, num, signClass, useAsync, Sym, Live, costUsd } from '../components/ui';
...
export default function PositionsView({ health }: { health?: any }) {
  const { data, loading } = useAsync<any[]>(Positions, [], 3000);
  const acct = useAsync<any>(Account, [], 3000);
  const rows = data || [];
        ...
        <>
          ...
          {(hasCash || rows.length > 0) && (
            <table className="pos-table">
              <thead>
                <tr><th>Symbol</th><th>Class</th><th>Qty</th><th>Cost</th><th>Value</th><th>P/L</th></tr>
              </thead>
              <tbody>
                {hasCash && (
                  <tr>
