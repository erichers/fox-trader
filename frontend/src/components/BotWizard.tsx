...
export default function BotWizard({ preset, onClose, onCreated }: { preset?: BotPreset; onClose: () => void; onCreated?: () => void }) {
  ...
  const [trailing, setTrailing] = useState(12);
      ...
      <div className="modal" style={{ width: 560, borderColor: 'var(--accent)' }} onClick={(e) => e.stopPropagation()}>
        ...
        {step === 1 && (
          <div style={{ marginTop: 12 }}>
            ...
            <Field label="Trade type" hint="This desk is options-only. Shares are blocked.">
        ...
        {step === 2 && isOpt && (
          <div style={{ marginTop: 12 }}>
            ...
            <Field label="Expiration" hint="Weekly snaps to a 2–10 trading-day expiry (aim ~7). Monthly is the far edge of that window, not 30 days.">
        ...
        {step === stepFor('money', isOpt) && (
          <div style={{ marginTop: 12 }}>
            ...
            <div className="row" style={{ gap: 16 }}>
              <Field label="Take profit %" hint="0 = no cap. Winners ride the trail after the +20% lock."><input type="number" value={takeProfit} onChange={(e) => setTakeProfit(+e.target.value)} style={{ width: 90 }} /></Field>
              <Field label="Stop loss %" hint="Cut the loser. Default 10% of premium."><input type="number" value={stopLoss} onChange={(e) => setStopLoss(+e.target.value)} style={{ width: 90 }} /></Field>
              <Field label="Trailing stop %" hint="Give-back from peak after the trade is up 20%."><input type="number" value={trailing} onChange={(e) => setTrailing(+e.target.value)} style={{ width: 90 }} /></Field>
            </div>
