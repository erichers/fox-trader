...
export const RISK_FIELDS: { key: RiskField; label: string; money?: boolean; suffix: string; help: string; placeholder?: string }[] = [
  ...
  { key: 'stop_loss_pct', label: 'Stop loss', suffix: '%', help: 'Fixed loss that closes the trade. Swing default is 10% of premium: cut the loser, do not wait for 35%.' },
  { key: 'trailing_stop_pct', label: 'Trailing stop', suffix: '%', help: 'Give-back from the best price reached, but only after the trade has been up 20%. Until then the only exit is the hard stop, so a good setup has room to develop.' },
];
...
/** One-line plain summary of a resolved risk set: "$1,000 per trade, tp none / sl 10% / trail 12%".
