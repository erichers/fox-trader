...
/** Options are 100-share contracts. Premium is per share; cash = qty × price × 100. */
export function unitMult(row: any): number {
  return String(row?.asset_class || '').toLowerCase() === 'option' || row?.occ_symbol ? 100 : 1;
}
export function notionalUsd(qty: any, price: any, row?: any): number | null {
  const q = Number(qty), px = Number(price);
  if (!(q > 0) || !(px > 0)) return null;
  return Math.round(q * px * unitMult(row) * 100) / 100;
}
export function costUsd(row: any): number | null {
  if (row?.cost_usd != null && Number.isFinite(Number(row.cost_usd))) return Number(row.cost_usd);
  return notionalUsd(row?.qty, row?.avg_cost ?? row?.entry ?? row?.entry_price ?? row?.filled_price ?? row?.limit_price, row);
}
